...
def test_path_in_joins_state_dir():
    p = pr.path_in(Path("/tmp/statedir"))
    assert p.name.endswith(".json")
    assert p.parent == Path("/tmp/statedir")
...
def test_load_string_stamp_dropped_not_coerced(tmp_path):
    ...
    out = pr.load(p)
    assert "m" not in out.get("amd", {})  # junk entry dropped
